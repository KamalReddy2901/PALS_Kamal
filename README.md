...
- **Saves simulations to an Interventions Tracker** that persists across the session, giving planners a log of proposed, approved, and executed interventions with predicted vs. baseline ΔT
...
Zustand manages a 50-state undo/redo history for the simulator and persists interventions in memory across routes, so a save on the ward page immediately appears in the tracker without any re-fetch. The choropleth uses `react-leaflet` with approximate 10-point polygons for each ward at real BBMP geographic coordinates, colored with a single-hue Vermillion ramp interpolated across six stops.
...
| Citizen View — public map + heat risk lookup + plain-language risk band | Separate `/citizen` route with ward name search, today/tomorrow peak temp, EXTREME/HIGH/MODERATE/LOW risk message, interventions list | `app/citizen/page.tsx` |
...
| Interventions Tracker — log proposed/approved/executed interventions | Full `/interventions` page with table, status filter, sort, summary stats; "Save to Tracker" from simulator; "+ New Intervention" sheet | `app/interventions/page.tsx`, `lib/store.ts` (addIntervention) |
| Semantic HTML, no broken links | `<header>`, `<nav>`, `<main>`, `<aside>`, `<section>`, `<article>`, `aria-label`, `aria-current`, `role` throughout; all internal links verified in static build | All page and component files |
| Zero console errors | Tooltip value type guard, sidebar Math.random hydration fix, IST timezone on all date renders, Vercel Analytics removed | `components/forecast-chart.tsx`, `components/ui/sidebar.tsx`, `app/layout.tsx` |
